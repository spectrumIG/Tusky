...
/**
 * Use Logcat to read Android logs into a file.
 * @throws IOException
...
fun getLogFile(context: Context): Uri {
    ...
    val process = Runtime.getRuntime()
            .exec(arrayOf("logcat", "-d", "-T", "1500", "-f", logFile.absolutePath))
    try {
        process.waitFor()
...
class ProcessExecutionException(val exitCode: Int, val stderr: String) : RuntimeException(stderr)